...
$APPLICATION->SetTitle("Комментарии к свойствам");
...
if ( $_GET["SECTION_ID"] ){
	...
	$listFile = $_SERVER["DOCUMENT_ROOT"]."/tools/files/comments_prop_".$IBLOCK_ID.".txt";
	...
	if ( is_array( $_POST['prop'] )){
		...
		foreach ( $_POST['prop'] as $key=>$val){
			$arPropData[$_GET["SECTION_ID"]][$key] = $_POST['comment'][$key];
		}
		...
		<?if ( $_GET["SECTION_ID"] ):?>
			...
			if($arSection = $res->GetNext()):?>
				...
				while ($prop_fields = $properties->GetNext()){
					...
					$arPropName[$prop_fields["ID"]] = $prop_fields["NAME"];
				}
				?>
				...
				<form action='?IBLOCK_ID=<?=$IBLOCK_ID?>&SECTION_ID=<?=$arSection["ID"]?>' method="post">
					<?if (is_array( $arPropData[$_GET["SECTION_ID"]] )):?>
						<b>Комментарии к  свойствам:</b><br><br>
						
						<?foreach ( $arPropData[$_GET["SECTION_ID"]] as $codeProp => $valProp):?>
							<div class="prop_comment">
								<span><?=$arPropName[$codeProp]?>:</span> 
								<textarea name="comment[<?=$codeProp?>]"><?=$valProp?></textarea>
							</div>
						<?endforeach;?>
					...
					<a href="#" id="show_props">Добавить/редактировать свойства</a>
							...
							<ul class="prop_list">
								<?foreach ( $arPropValue as $prop_fields):?>
									...
									<?if ( $prop_fields["SORT"] > 5 || $prop_fields["CODE"] == 'BREND'):?>
										<li>
											<label>
												<input type="checkbox" class="check" name="prop[<?=$prop_fields["ID"]?>]" <?if ( array_key_exists($prop_fields["ID"], $arPropData[$_GET["SECTION_ID"]]) ):?>checked<?endif;?>>  <?=$prop_fields["NAME"]?>
											<label>
										</li>
									<?endif;?>
