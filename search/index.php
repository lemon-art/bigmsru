...
<?
/*
$APPLICATION->IncludeComponent(//это для того чтоб автоматом раскладка вставала
"bitrix:search.page",
"catalog",
Array(
   "TAGS_SORT" => "NAME",
		...
		//if ($USER->IsAdmin()){
			if(preg_match('/[a-zA-Z]/', $_REQUEST["q"])){
				...
				foreach($arResult["QUERY_USER_LANG"] as $key => $value){		
					...
					{
						unset($arResult["QUERY_USER_LANG"][$key]);
						continue;
					}
				}
			}
...
function __recursivRenderMenu($ar_Items) {
    ...
    global $APPLICATION;
    global $count_sections;
    $res = '';
    foreach ($ar_Items as $ar_Value) {
        $page = $APPLICATION->GetCurPageParam("section_id=".$ar_Value['ID'], array("section_id"));
        if ( count($ar_Value['SUB_SECTION']) > 0 ) {
            $active = '';
            if ($_GET['section_id'] == $ar_Value['ID'] && strlen($_GET['section_id']) > 0) {
...
?>
	...
	<?//global $USER;
	//if ($USER->IsAdmin()):
		$arResult["QUERY_USER"] = explode(" ", $_REQUEST["q"]);		
		
		foreach($arResult["QUERY_USER"] as $key => $value){		
	...
	?>
		...
		$( document ).ready(function() {
			...
			var result_query_arr = result_query.split(' ');
			//var original_q_arr = original_q.split(' ');
			var isGetInSpeller = '<?=$isGetInSpeller?>';
			var result_query_new = "";
			
			
			/*console.log(result_query_arr);
			console.log(isGetInSpeller);*/
			...
			if(isGetInSpeller == "ok"){
				$.each( result_query_arr, function( key, value ) {
					<?foreach($arKeys as $key => $value):?>
						if(key == <?=$value?>){
							if((result_query_new.indexOf('<?=$arResult["QUERY_USER"][$value]?>') + 1) == 0) {
								result_query_new += '<?=$arResult["QUERY_USER"][$value]?>'+" ";
							}
						}else if(value.match(/([а-яА-Я]+)$/)){
							if((result_query_new.indexOf(result_query_arr[key]) + 1) == 0) {
								result_query_new += result_query_arr[key]+" ";
							}
						}
					<?endforeach;?>
				});
