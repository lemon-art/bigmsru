...
if($this->startResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
	...
	if($arParams["ELEMENT_ID"] > 0)
	{
		...
		if($obElement = $rsElement->GetNextElement())
		{
			...
			$arFilter = Array("IBLOCK_ID"=>$IBLOCK_GIFT_ID, "ACTIVE"=>"Y");
			$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect); //получаем список всех доступных подарков
			while($ob = $res->GetNextElement()){ 
				
				$arGiftProps = $ob->GetProperties();
				$arGifts[] = $arGiftProps;
			}
			...
			if ( count ( $arGifts ) > 0){
				...
				foreach ( $arGifts as $key => $arGift ){
					...
					foreach ( $arProprs as $keyProp => $arProp){
						if ( $arProp["VALUE"] == $arResult["PROPERTIES"][$keyProp]["VALUE"] ){
							$arItog[$key] = 1; //подарок найден
						}
					}
				...
				if ( count ( $arItog ) > 0){ //если есть подарок то получаем данные по нему
					foreach ( $arItog as $key=>$val)
						$keyItog = $key;
					$gift_ID =  $arGifts[$keyItog]['GIFT']['VALUE'];
					$arSelect = Array("ID", "IBLOCK_ID", "NAME", "PREVIEW_PICTURE", "DETAIL_PICTURE", "DETAIL_PAGE_URL");
					$arFilter = Array("IBLOCK_ID"=>$arResult["IBLOCK_ID"], "ID"=>$gift_ID);
					$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect); 
					if($ob = $res->GetNextElement()){ 
						$arFields = $ob->GetFields();
						$arResult["GIFT"] = $arFields;
					}
