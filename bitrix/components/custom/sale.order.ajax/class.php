...
class SaleOrderAjax extends \CBitrixComponent
{
		...
		foreach ($propertyCollection as $property)
		{
			...
			if ($arProperty["IS_ZIP"] == "Y" && !$isProfileChanged)
			{
				...
				if ($locId > 0)
				{
					...
					$locValue = $orderProperties[$locId];

					if ($loadFromProfile || $this->request->get('PROFILE_ID') === '0'|| $this->request->get('location_type') == 'code')
						$locValue = CSaleLocation::getLocationIDbyCODE($locValue);

					$rsZipList = CSaleLocation::GetLocationZIP($locValue);
	...
	{ 
		...
		//получаем сами свойства заказа
		$arUserProps = Array(); //массив с конечными свойствами
		$arProps = $this->getPropertyValuesFromRequest(); 
		foreach ($arProps as $keyProp => $arProp ){
			if ($arOrderProps = CSaleOrderProps::GetByID($keyProp)){
				$arUserProps[$arOrderProps["CODE"]] = $arProp;
			}
		}
		
		
		file_put_contents($_SERVER["DOCUMENT_ROOT"]."/tools/files/test.txt", "<?$"."userProps = ".var_export($arUserProps,true).";?>");
		...
		$arAuthResult = $user->Add(array(
			...
			"NAME" => $arUserProps['NAME'],
			//"LAST_NAME" => $arUserProps['LAST_NAME'],
			"PASSWORD" => $userData['NEW_PASSWORD'],
			"CONFIRM_PASSWORD" => $userData['NEW_PASSWORD_CONFIRM'],
			"EMAIL" => $userData['NEW_EMAIL'],
			"GROUP_ID" => $userData['GROUP_ID'],
			"ACTIVE" => "Y",
			"PERSONAL_PHONE" => $arUserProps['PHONE'],
			"PERSONAL_ZIP" => $arUserProps['ZIP'],
			"PERSONAL_STREET" => $arUserProps['ADDRESS'],
			"WORK_COMPANY" => $arUserProps['COMPANY'],
			"WORK_STREET" => $arUserProps['COMPANY_ADR'],
			"WORK_DEPARTMENT" => "ИНН: ". $arUserProps['INN'],
			"WORK_POSITION" => "КПП: ". $arUserProps['KPP'],
			"WORK_PHONE" => $arUserProps['PHONE'],
			"LID" => $this->context->getSite()
		));
		
		if (  $arUserProps['COMPANY'] ){
			$arAuthResult["NAME"] = $arUserProps['COMPANY'];
		}
