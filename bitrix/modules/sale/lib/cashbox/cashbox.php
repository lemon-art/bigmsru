...
abstract class Cashbox
{
	 ...
	 * @return Result
	 */
	public static function validateSettings($data, $modelId = 0)
	{
		$result = new Result();

		$requiredFields = static::getRequiredFields($modelId);
		...
		foreach ($data as $code => $value)
		{
			if (isset($requiredFields[$code]))
			{
				...
				if ($isError)
				{
					$result->addError(new Main\Error(Loc::getMessage('SALE_CASHBOX_VALIDATE_ERROR', array('#FIELD_ID#' => ToLower($requiredFields[$code])))));
				}
			}
		}
	...
	private static function getRequiredFields($modelId = 0)
	{
		...
		{
			if (isset($group['REQUIRED']) && $group['REQUIRED'] === 'Y')
				$result[$groupId] = $group['LABEL'];
		}
