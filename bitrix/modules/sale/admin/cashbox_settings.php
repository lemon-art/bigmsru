			...
			if ($settings)
			{
				foreach ($settings as $group => $block)
				{
					...
					foreach ($block['ITEMS'] as $code => $item)
					{
						...
						$result .= '<td width="45%" class="'.$className.'">'.$item['LABEL'].':</td><td width="55%" valign="top" class="adm-detail-content-cell-r">'.Input\Manager::getEditHtml('SETTINGS['.$group.']['.$code.']', $item, $value).'</td></tr>';
					}
				}
			}
		}
