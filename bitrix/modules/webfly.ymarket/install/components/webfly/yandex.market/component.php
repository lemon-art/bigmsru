...
// PROPDUCT_PROP - dopolnitelnye svojstva tovarov vyvodimye v opisanii
            $propsArray = array("LOCAL_DELIVERY_COST_OFFER", "STORE_OFFER", "STORE_PICKUP", "ADULT", "PREFIX_PROP", "PROPDUCT_PROP", "DELIVERY_OPTIONS_EX", "STORE_DELIVERY", "OUTLETS", "AGE_CATEGORY", "BID", "CBID", "FEE", "CPA_OFFERS", "EXPIRY", "WEIGHT", "DIMENSIONS", "RECOMMENDATION");
...
//nakoplenie v peremennuyu
                            ...
                            if ($outlets)
                            {
                                foreach ($outlets as $outKey => $outProp)
                                {
                                    foreach ($outProp as $outk => $outv)
                                    {
                                        $productAmount = "";
                                        $amountPropCode = "";
                                        if (substr_count($outv, "WFYMAMOUNT") > 0)//value from amount
                                        {//amount
                                            switch ($outk) {
                                                case "0"://amount ID
                                        ...
                                        if (substr_count($outv, "WFYMAMOUNT") == 0 and substr_count($outv, "WFYMPROP") == 0)
                                        {//write
                        ...
                        case "RECOMMENDATION":
                            ...
                            while ($ob_props = $arProps->GetNext()) {
                                $recItem = $ob_props["VALUE"];
                                if (!empty($recItem))
                                {
                                    if (empty($arOffer[$propVal]))
                                        $arOffer[$propVal] = $recItem;
                                    else
                                        $arOffer[$propVal].="," . $recItem;
                                }
                                unset($recItem);
...
// PROPDUCT_PROP - dopolnitelnye svojstva predlozhenij vyvodimye v opisanii
                $propsArrayOffers = array("LOCAL_DELIVERY_COST_OFFER", "STORE_OFFER", "STORE_PICKUP", "ADULT", "PREFIX_PROP", "OFFER_PROP", "DELIVERY_OPTIONS_EX", "STORE_DELIVERY", "OUTLETS", "AGE_CATEGORY", "BID", "CBID", "FEE", "CPA_OFFERS", "EXPIRY", "WEIGHT", "DIMENSIONS", "RECOMMENDATION");
//esli vybran algoritm opredeleniya dostupnosti tovara iz svojstva
                if ($arParams["AVAILABLE_ALGORITHM"] == "PROP_ALGORITHM")
