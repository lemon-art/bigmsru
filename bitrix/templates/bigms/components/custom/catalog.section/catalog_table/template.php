...
<table  class="catalog_list catalog_table">
<?
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	<tr class="item" id="<? echo $strMainID; ?>">
		<td class="img_block">
			<div class="labels">
				<?
			...
			<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" itemprop="image" src="<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>"><?
				if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT'])
				{
				?>
					<div id="<?if ($arItem['SECOND_PICT']){echo $arItemIDs['SECOND_PICT'];}else{ echo $arItemIDs['DSC_PERC'];}?>" class="bx_stick_disc right bottom" style="display:<? echo 'none'; ?>;"></div>
				<?
				}
				if ($arItem['LABEL'])
				{
                ...
                <? } else {?>
                    <img itemprop="image" class="image_rework_table" itemprop="image" src="/bitrix/templates/bigms/images/logo_bw.png" alt="<? echo $imgTitle; ?>" title="<? echo $imgTitle; ?>">
                <? } ?>
			</a>
		</td>
		...
		<td class="price_block">
			<div id="<? echo $arItemIDs['PRICE']; ?>" itemprop="price" class="bx_price"><?
			if (!empty($minPrice))
			{
		...
		<td class="bx_catalog_item_controls"><?
			if ($arItem['CAN_BUY'])
			{
				...
				<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
					<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium buy_button" href="javascript:void(0)" rel="nofollow" onmousedown="try { rrApi.addToBasket(<?=$arItem['ID']?>) } catch(e) {}" onclick="yaCounter31721621.reachGoal('basket');" data-id="<?=$arItem['ID']?>"><?
					...
					<div class="byclick">
						<a href="#" data-id="<?=$arItem["ID"]?>" data-iblock="<?=$arParams["IBLOCK_ID"]?>" class="buy_one_button_table" onclick="yaCounter31721621.reachGoal('catalog-oneclickbuy-button');">купить в 1 клик</a>
					</div>
