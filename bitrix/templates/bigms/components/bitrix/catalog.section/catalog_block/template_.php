...
<div class="catalog_list catalog_block">
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	<div class="bx_catalog_item_container <?if (!$arItem['CAN_BUY']){echo 'small';}?>" id="<? echo $strMainID; ?>">
		...
		<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="img" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>')" title="<? echo $imgTitle; ?>"><?
		...
		if($arItem["CATALOG_QUANTITY"] == 0){
			echo '<div class="availability no">Наличие уточняйте</div>';
		}
		else{
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		<div class="bx_catalog_item_controls"><?
			...
			<div class="do_block small">
				<a id="<? echo $arItemIDs['COMPARE_LINK']; ?>" data-id="<?=$arItem["ID"]?>" class="bx_bt_button_type_2 bx_medium compare_button" href="javascript:void(0)" onclick="yaCounter31721621.reachGoal('add_to_compare'); return true;"></a>

				<a class="share_button" data-id="<?=$arItem["ID"]?>" href="javascript:void(0)" onclick="add2delay('<?=$arItem["ID"]?>','<?=$arItem["CATALOG_PRICE_ID_1"]?>','<?=$arItem["CATALOG_PRICE_1"]?>','<?=addslashes($arItem["NAME"]);?>','<?=$arItem["DETAIL_PAGE_URL"]?>'); yaCounter31721621.reachGoal('add_to_bookmarks');"></a>
			</div>
