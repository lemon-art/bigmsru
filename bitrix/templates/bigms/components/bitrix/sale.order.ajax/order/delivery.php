			...
			else // stores and courier
			{
				...
				?>
					...
					<style>
						...
						.content input[type="radio"]+label.label_order_input {
    background: transparent;
    position: relative;
}
.content input[type="radio"]+label.label_order_input:before {
    content: '';
    position: absolute;
    left: 0;
    top: 0;
...
.content input[type="radio"]:checked+label.label_order_input:after {
    content: '';
    position: absolute;
    left: 3px;
    top: 3px;
    ...
    border-radius: 50%;
			...
			if($arDelivery["ID"] == 1 && $arDelivery["CHECKED"] == "Y"){
				$option = '';
				$desc = '';
				...
				{
					$i++;
					//$option .= '<option class="store_row" data="desc'.$ar_fields["ID"].'" value="'.$ar_fields["NAME"].'">'.$ar_fields["NAME"].'</option>';
					$radioOption .= '<input type="radio" onchange="changeRadioOption(this)" data="desc'.$ar_fields["ID"].'" id="radioOption'.$i.'" name="radioOption" value="'.$ar_fields["NAME"].'"><label class="label_order_input" for="radioOption'.$i.'">'.$ar_fields["NAME"].'</label><br>';
					if(!empty($ar_fields["PREVIEW_TEXT"])){
						$desc .= '<div class="store_desc desc'.$ar_fields["ID"].' hidden">'.$ar_fields["PREVIEW_TEXT"].'<br /></div>';
					}
					else{
						$desc .= '<div class="store_desc desc'.$ar_fields["ID"].' hidden"></div>';
					}
				}
				?>
				
				<!--select id="store_list" onchange="setChangeStore(this);" class="select">
					<option class="store_row" value="0">Выберите склад</option>
					<?=$option;?>					
				</select-->
				<?=$radioOption?>
				<?
...
<script>
	function write_store_input(val){	
