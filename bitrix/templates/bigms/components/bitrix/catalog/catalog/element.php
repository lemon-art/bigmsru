...
if ($USER->IsAdmin()){ 
	...
	$APPLICATION->SetPageProperty("is_catalog_intro", "Yes");
...
if($arParams["IBLOCK_ID"] == 10 || $arParams["IBLOCK_ID"] == 12){?>
	...
	<?if($count_similar > 0):?>	
		<div class="buy_with_this clearfix">
			...
			<?if($count_similar > 0 && $count_similar <= 5):?>
	...
	$count_similar = count($arVIEWED_PRODUCT);
	$similarFilter2 = Array("ID" => $arVIEWED_PRODUCT, "INCLUDE_SUBSECTIONS" => "Y");
	?>

	
	<?if( $count_similar > 1):?>	
		<div class="buy_with_this clearfix lite_block">
			<div class="title_block"><div data-kol="<? echo $count_similar; ?>">Просмотренные товары</div></div>	
			<?if($count_similar > 0 && $count_similar <= 4):?>
				<div class="catalog_block not_carusel lite">
			<?else:?>
				<div class="flexslider flexslider_run lite">
			<?endif?>
				<ul class="catalog_list catalog_block catalog_carusel slides lite">
					<?$APPLICATION->IncludeComponent("bitrix:catalog.section", "carusel_lite", Array(
						"COMPONENT_TEMPLATE" => ".default",
							"IBLOCK_TYPE" => "1c_catalog",
							"IBLOCK_ID" => 10,
							"SECTION_ID" => "",
