...
foreach($arResult as $arItem):?>
	...
	<?else:?>
		...
		<?if ($arItem["PERMISSION"] > "D"):?>
			...
			<?if ($arItem["DEPTH_LEVEL"] == 1):?>
				<li class="<?if ($arItem["SELECTED"]):?>active<?else:?>root-item<?endif?>" data-link="<?=$arItem["LINK"]?>"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a></li>
			<?else:?>
				<li <?if ($arItem["SELECTED"]):?> class="active"<?endif?> data-link="<?=$arItem["LINK"]?>"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a></li>
			<?endif?>
