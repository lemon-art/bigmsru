	...
	else
	{
			...
			<div id="form_comment_" style="display:none;">
				...
				<div class="blog-comment-fields">
					...
					<div class="blog-comment-buttons">
						<input tabindex="10" value="<?=GetMessage("B_B_MS_SEND")?>" type="button" name="sub-post" id="post-button" onclick="submitComment()" class="btn">
					</div>
				</div>
					...
					<div class="blog-comment-info">
						...
						else
						{
							/* if($comment["CAN_EDIT"] == "Y"){
								...
								$comment["AuthorName"] = 'Большой мастер';
							} */
							
							if(is_array($comment["arUser"])){
								$arGroups = CUser::GetUserGroup($comment["arUser"]["ID"]);
							...
							?>
							<div class="blog-author <?=$class_color?>"><?=$comment["AuthorName"]?></div>
							<?
						}
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment"><a class="bx_medium bx_bt_button" href="javascript:void(0)" onclick="return showComment('0')"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a></div>
				<a name="0"></a>
				<?
