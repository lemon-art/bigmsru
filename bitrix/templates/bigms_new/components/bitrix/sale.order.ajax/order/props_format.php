		...
		if (!empty($arSource))
		{
			?>
				<div>
					...
					{
						$prop_id = intval(intval($arProperties["ID"]));
						$hidden = '';
						if($prop_id == 17 || $prop_id == 18 || $prop_id == 4 || $prop_id == 5 || $prop_id == 14 || $prop_id == 15){
							$hidden = 'hidden';
						}
						?>
						<div class="field <?=$hidden?>" data-property-id-row="<?=$prop_id?>">
						...
						if ($arProperties["TYPE"] == "CHECKBOX")
						{
							...
							<div class="bx_block r1x3 pt8">
								...
								<?if ($arProperties["REQUIED_FORMATED"]=="Y"):?>
									<span class="bx_sof_req asterisk">*</span>
								<?endif;?>
							</div>
						...
						{
							?>
							<div class="bx_block r1x3 pt8 label">
								<?=$arProperties["NAME"]?>
								<?if ($arProperties["REQUIED_FORMATED"]=="Y"):?>
									<span class="bx_sof_req asterisk">*</span>
								<?endif;?>
						...
						elseif ($arProperties["TYPE"] == "SELECT")
						{
							...
							<div class="bx_block r3x1">
								<select name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" size="<?=$arProperties["SIZE1"]?>" class="select">
									<?
						...
						elseif ($arProperties["TYPE"] == "MULTISELECT")
						{
							...
							<div class="bx_block r3x1">
								<select multiple name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" size="<?=$arProperties["SIZE1"]?>" class="select">
									<?
									foreach($arProperties["VARIANTS"] as $arVariants):
									?>
										<option value="<?=$arVariants["VALUE"]?>"<?if ($arVariants["SELECTED"] == "Y") echo " selected";?>><?=$arVariants["NAME"]?></option>
