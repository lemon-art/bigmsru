...
function changeRadioOption(e){
	var elems=document.getElementsByClassName('store_desc');
	for(var i=0; i<elems.length; i++){
		elems[i].style.display='none';
	}
	document.getElementsByClassName(e.attributes.data.value)[0].style.display = 'block';

	var value = e.value;
	if (value.length > 1) {
		document.getElementById('ORDER_PROP_17').value = value;
	}
}