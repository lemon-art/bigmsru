...
function updateBasketTable(basketItemId, res)
{
		...
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('discount_value_' + id))
					if ( item.DISCOUNT_PRICE_FORMATED )
						BX('discount_value_' + id).innerHTML = item.DISCOUNT_PRICE_FORMATED;
					else	
						BX('discount_value_' + id).innerHTML = item.PRICE_FORMATED;
				...
				if (BX('QUANTITY_' + id))
				{
					...
					all_count = all_count + item.QUANTITY*1;
				}
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('top_total_price'))
			BX('top_total_price').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;');
		
		if (BX('all_count'))
			BX('all_count').innerHTML = all_count;	
		
		if (BX('top_basket_number'))
			BX('top_basket_number').innerHTML = all_count;	
		
		if (BX('top_basket_number_text'))
			BX('top_basket_number_text').innerHTML = '<span class="status-dropdown__number">' + all_count +'</span> ' + declOfNum(all_count,['товар','товара','товаров']);				
			
			
		if (BX('all_count_text'))
			BX('all_count_text').innerHTML = declOfNum(all_count,['товар','товара','товаров']);	
			
		
		if (BX('top_basket_summ'))
			BX('top_basket_summ').innerHTML = 'на сумму <span class="status-dropdown__price">' + res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;') + '</span>';
		...
		BX.ajax({
			...
			onsuccess: function(result)
			{
				...
				BX.ajax({
					...
					{
						alert('123');
						result('.header__status').html( result );
					}
				});
