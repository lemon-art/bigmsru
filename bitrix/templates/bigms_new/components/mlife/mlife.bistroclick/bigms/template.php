...
<div class="mlf_product">
	<div class="imager">
		<div class="imageWp">
			<?
			$res = CIBlockElement::GetByID($arResult["PRODUCT_ID"]);
			if($ar_res = $res->GetNext()){				
				$arResult["TOVAR"]["DETAIL_PICTURE"] = $ar_res["DETAIL_PICTURE"];
				$arResult["TOVAR"]["NAME"] = $ar_res["NAME"];
			}
			?>
		...
		<div class="vlgen">
			...
			<div class="pricen">
				<div class="priceFirst">
					<?
					if($arResult["OFFERS"]){
						echo GetMessage('MLIFE_CAT_BK_OT').' '.CurrencyFormat(round($arResult['PRICE_MIN']),$arParams['CURRENCY_ID']);
					}
					else{
...
<div class="mlfform">
	...
	if(is_array($arParams['FIELD_SHOW']) && count($arParams['FIELD_SHOW'])>0){
		...
		if(isset($arResult['LABEL'][$value])) {
			...
			$fieldname = GetMessage("MLIFE_CAT_BK_FIELD_REQ_".strtoupper($value));
		}
		?><?
			if($value=='mess') { 
				$messfield = true;
			...
			?>
				<div class="field">
					<label class="label" for="<?=$value?>"><?=$fieldname?><?if($arResult['SEND_REQ'][$value]==1) echo '<span class="asterisk">*</span>';?>:</label>
					<?if($value!='addfield_'.$arResult['LOC_ID']){?>
						<div><input id="<?=$value?>" type="text" name="<?=$value?>" value="<?=$arResult['SEND'][$value]?>"/></div>
					<?}else{?>
	...
	<?if(is_array($arParams["FIELD_DELIVERY"]) && count($arParams["FIELD_DELIVERY"])==1){
		...
		?><div class="field"><label for="delivery"><?=GetMessage("MLIFE_CAT_BK_DELIVERY")?></label>
	...
	<?if(is_array($arParams["FIELD_PAYSYSTEM"]) && count($arParams["FIELD_PAYSYSTEM"])==1){
		...
		?><div class="field"><label for="paysystem"><?=GetMessage("MLIFE_CAT_BK_PAYSYSTEM")?></label>
	...
	<?if($messfield){?>
	<div class="field"><label for="mess"><?=GetMessage("MLIFE_CAT_BK_FIELD_REQ_".strtoupper('mess'))?><?if($arResult['SEND_REQ']['mess']==1) echo '<span>*</span>';?>:</label>
	...
	<div class="mlf_capcha">
		<div class="label"><?=GetMessage("MLIFE_CAT_BK_KAPCHA_LABEL")?><span class="asterisk">*</span>:</div>
...
echo bitrix_sessid_post('bistrclick_sessid');
	...
	<input type="submit" class="mlfsubmitbk buy_button" value="<?=GetMessage('MLIFE_CAT_BK_SEND_BUTTON')?>" onclick="yaCounter31721621.reachGoal('catalog-oneclickbuy-submit'); return true;" />
	</form>
...
die();
	...
	$(document).ready(function(){
		$(".byclick .buy_one_button").each(function() {
			$(this).attr('href','<?=$arResult['REF_START']?>?formclick=1&referer=<?=$key?>&pr_id='+$(this).attr("data-id")+'&pr_iblock='+$(this).attr("data-iblock"));
		});
		$(".byclick .buy_one_button_table").each(function() {
			$(this).attr('href','<?=$arResult['REF_START']?>?formclick=1&referer=<?=$key?>&pr_id='+$(this).attr("data-id")+'&pr_iblock='+$(this).attr("data-iblock"));
		});
	});
	
		$('.byclick .buy_one_button').fancybox({
			'type': 'iframe',
			'width': 350,
			'height': 300,
			'minHeight': 365,
		...
		$('.byclick .buy_one_button_table').fancybox({
			'type': 'iframe',
			'width': 350,
			'height': 300,
			'minHeight': 365,
...
<body>
	<? echo GetMessage('MLIFE_CAT_BK_ERROR_KEY');?> 
</body>
</html>
<?
die();
