...
$ids = $arParams["IBLOCK_ID"]==10?12:10;
...
//foreach($main_section as $ids) {
    ...
    $filter = $recFilter + Array('ACTIVE' => 'Y', "IBLOCK_ID" => $ids);
    $res = /*CCatalogProduct*/CIBlockElement::GetList(Array(), $filter, false, false, Array("ID", "NAME", "IBLOCK_ID","PREVIEW_PICTURE", "DATE_CREATE", "DETAIL_PAGE_URL", "PROPERTY_NEW", "PROPERTY_RASPRODAZHA", "PROPERTY_NOVINKA"));
    ...
    while($ar = $res->GetNextElement())
    {
        ...
        $item["PRICES"] = CIBlockPriceTools::GetItemPrices($item["IBLOCK_ID"], $arResult["PRICES"], $item, $arParams['PRICE_VAT_INCLUDE']);
        ...
        if (!empty($item['PRICES'])) {
            ...
            $price_ib = array();
            $sku = CCatalogSKU::getOffersList([$item['ID']]);
            ...
            unset($sku);
            unset($sku_items);
            $item['MIN_PRICE'] = array('VALUE'=>reset($price_ib));
        }
        $item['CAN_BUY'] = 1;
        $item['PRICE'] = CPrice::GetBasePrice($item['ID']);
