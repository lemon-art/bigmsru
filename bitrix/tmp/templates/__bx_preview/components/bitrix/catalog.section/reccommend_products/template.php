...
<div id="rec" style="display: block;">
    <div class="buy_with_this clearfix">
        <div class="title_block"><div data-kol="<?= $count_rec_ar; ?>">Рекомендованные товары</div></div>
        
<?if(count($arResult['ITEMS']) > 0 && count($arResult['ITEMS']) <= 4):?>
    <div class="catalog_block not_carusel">
<?else:?>
    <div class="flexslider flexslider_run">
<?endif?>

<ul class="catalog_list catalog_block catalog_carusel slides">
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	if (isset($arItem['PRICE']['PRICE']))
		$minPrice = $arItem['PRICE']['PRICE'];
	...
	<div class="bx_catalog_item_container <?if (!$arItem['CAN_BUY']){echo 'small';}?>" id="<? echo $strMainID; ?>">
		...
		<div class="in">
			...
			<a class="bx_catalog_img_link" id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"><?
            ...
            <?
            if (isset($arItem["PRICE"]["PRODUCT_QUANTITY"])) {
