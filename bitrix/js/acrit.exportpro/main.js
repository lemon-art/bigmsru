...
function ShowResolveBlock( value ){
    ...
    $(value).find("option").each(function(ix,elem){
    	
    	
    	
    	if(elem.value==data['data_value']){
    		
    		
    		if( typeof $( elem ).attr( 'data-property' ) !== 'undefined' ){
    	    	var property= $( elem ).attr('data-property');
    			$( value ).parents(".fieldset-item").find("[data-dependent]").each(function(ix,el){
    				
    				 if( typeof $( el ).attr( 'data-dependent' ) !== 'undefined' ){
    					 
    					 if($( el ).attr( 'data-dependent' )==property){
...
function ShowMinimumOfferPriceCode( value ){
	
	if( typeof $( value ).attr( 'checked' ) !== 'undefined' ){
		$( value ).parent(".minimum_offer_price").find(".minimum_offer_price_code").show();
    }
    else{
    	$( value ).parent(".minimum_offer_price").find(".minimum_offer_price_code").hide();
    }
}