yeni_ipep = {
    ...
    init: function () {
            ...
            for (i in yeni_ipep.props_to_groups) {
                for (j in yeni_ipep.props_to_groups[i]) {
                    if (yeni_ipep.props_to_groups[i][j]['GROUP_ID'] != '') {
						...
						var main_group = yeni_ipep.props_to_groups[i][j]['MAIN_GROUP'];

						
                        if (typeof(yeni_ipep.addedGroups[group_id]) == 'undefined') {
                            newDom.after(yeni_ipep.add_group(false, group_id, group_name, false, true, main_group));
        ...
        $('body').on('change', '.yeni_ipep_table .group_header input[type=text], .yeni_ipep_table .group_header input[type=checkbox]', function (e) {
            var findedCnt = 0;
            var curVal = $(this).val();
       ...
       /* $('.yeni_ipep_can_move tr:not(.state-disabled, .info)').each(function () {
        ...
        if (typeof(yeni_ipep.props_to_groups) != 'undefined') {
            for (i in yeni_ipep.props_to_groups) {
                for (j in yeni_ipep.props_to_groups[i]) {
                    if (yeni_ipep.props_to_groups[i][j]['GROUP_ID'] != '') {
                        yeni_ipep.add_group(false, yeni_ipep.props_to_groups[i][j]['GROUP_ID'], yeni_ipep.props_to_groups[i][j]['GROUP_NAME'], yeni_ipep.props_to_groups[i][j]['GROUP_SECTION_ID'], yeni_ipep.props_to_groups[i][j]['MAIN_GROUP']);
    ...
    add_group: function (click_from, group_id, group_name, group_section_id, onlyReturn, main_group) {
        ...
        if (typeof(group_id) == 'undefined') {
			...
			if ( main_group == "Y"){
				...
				last.find('.group_header input.group_main').attr('checked', true);
			}
    ...
    saveGroups: function ($this) {
        ...
        $('.yeni_ipep_table').each(function () {
			...
			//стоит ли галочка основная группа
			if ( $(this).find('.group_main:checked').val() ){
				var main_group = "Y";
			}
			else {
				var main_group = "";
